...
public class FavStorageTest {
    ...
    void corruptedFileReadFile_expectFavListSize3() throws FileNotFoundException, CustomException {
        ...
        assertEquals("valid| 1", FavList.getFav(0).getDesc());
        assertEquals("valid 2", FavList.getFav(1).getDesc());
        assertEquals("valid 3", FavList.getFav(2).getDesc());
        assertEquals("valid 4", FavList.getFav(3).getDesc());
    }
