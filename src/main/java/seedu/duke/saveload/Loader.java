...
public class Loader {
    ...
    public static Loader load(String folderName, String fileName) throws FileNotFoundException {
        return new Loader(folderName, fileName);
    }

    private Loader(String folderName, String fileName) throws FileNotFoundException {
        ...
        for (int j = 0; j < height; j++){
            ...
            if (width >= 0) System.arraycopy(line, 1, entries[j], 0, width);
        }
    }

    /**
     * Get the String entry stored at the specified position in the table if it is present.
     * @param x_position the x position in the table from 1 to the table width
     * @param y_position the y position in the table from 1 to the table height
     ...
     */
    public Optional<String> get(int x_position, int y_position) throws IndexOutOfBoundsException {
        try {
